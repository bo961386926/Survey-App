...
-- 插入字典项（排口类型）
INSERT IGNORE INTO sys_dict_item (dict_id, item_label, item_value, sort_order) 
SELECT d.id, item_label, item_value, sort_order FROM sys_dict d, (
  SELECT 1 as dict_id, '雨水排口' as item_label, 'rainwater' as item_value, 1 as sort_order UNION ALL
  SELECT 1, '污水排口', 'sewage', 2 UNION ALL
  SELECT 1, '雨污混流口', 'mixed', 3 UNION ALL
  SELECT 1, '工业废水排口', 'industrial', 4
) items WHERE d.dict_code = 'outfall_type';
...
-- 初始化点位状态字典项
INSERT IGNORE INTO sys_dictionary_data (dict_id, dict_code, data_name, data_value, data_order, is_readonly) 
SELECT id, 'POINT_STATUS', data_name, data_value, data_order, is_readonly FROM sys_dictionary WHERE dict_code='POINT_STATUS';
